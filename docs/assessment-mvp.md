...
| Scheduled expiry cleanup is deployed and monitored. | Blocked | The daily schedule is active on public staging and the handler is locally exercised; verify the first hosted invocation and retain monitoring evidence. | Yes |
| Cleanup failure notifications reach a responsible person. | Blocked | `curiousphreak@gmail.com` is the approved recipient; configure and test the Cloudflare email notification before production promotion. | Yes |
